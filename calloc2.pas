PROGRAM calloc2(INPUT,OUTPUT,adr_file,con_file,alloc_file);
...
{ 
 ...
 MODIFIED BY:
    26-Sep-86  1.06	  PIR	Fixed collect_set - "Impossilbe address constraint" check
    14-Jan-86  1.05	  PIR	Fixed dump_block; pre-allocated addresses 620-7DF
...
the CVAX microprocessror.  ALLOC2 uses the output of ALLOC1 (adr_file, con_file) and generates 
        ...
        MCR line number-----+       |     +-------- second argument
			...
			if contraint type is D (delta) and first argument is greater than 0
				...
				third argument : delta; the physical address of the offset is
						  the base physical address plus delta
...
PROCEDURE INIT;
   VAR
	...
	BEGIN
		...
		END;
{ 1.05 pre-allocate 620 to 7DF }

	    FOR count1 := %x'620' TO %x'7DF' DO
		    p_adr[count1] := count1 + %x'10' ;
...
VAR
	...
	ones_msk, zero_msk, a_con_delta, t_adr, adrx, addres, delta, ref_delta, tmp_delta, abs_delta, abs_adr : UNSIGNED ;
	okay, end_of_range, a_con, two_a_con : BOOLEAN ;
	s_mcr, b_mcr : INTEGER;
	care_bits, s_delta, b_delta, s_ones_msk, b_ones_msk, s_zero_msk, b_zero_msk : UNSIGNED;
...
{ ***BEGIN*** Version 1.06 change to fix impossible address contraint checking }
			care_bits := UNOT(UOR(s_ones_msk,s_zero_msk));
			okay := FALSE;
			end_of_range := FALSE;
			t_adr := s_ones_msk;

			REPEAT
			   adrx := t_adr + b_delta - s_delta;
			   IF NOT ( (UAND(adrx,b_ones_msk) <> b_ones_msk) OR
				    (UAND( UNOT(adrx),b_zero_msk) <> b_zero_msk) ) THEN
			      okay := TRUE
			   ELSE IF UAND( (t_adr+1),care_bits) <> (t_adr+1)  THEN
			      end_of_range := TRUE
			   ELSE 
			      t_adr := t_adr + 1;
			UNTIL end_of_range OR okay;
...
{ ***END***** Version 1.06 change to fix impossible address contraint checking }
...
VAR
	...
	adrs,ref_adrs,block_adrs,zero,ones,mcr_delta,start_adrs,set_adrs : UNSIGNED ;
...
VAR
	...
	abs_adrs,mcr_adrs,comp_adrs,mcr_delta,rad3_adrs,col5 : UNSIGNED ;
		      ...
		      'D' : BEGIN
				...
				ELSE
				  BEGIN
				      ...
				      mcr_blk := UAND ( blk_msk , mcr[mcr_num].p_adr );
				      comp_blk := UAND ( blk_msk , mcr[comp_mcr].p_adr );
				      IF (( comp_adrs + mcr_delta ) <> mcr_adrs ) OR ( mcr_blk <> comp_blk ) THEN
					BEGIN
					    WRITELN ;
					    WRITELN ('VERIFY ERROR ');
					    WRITELN (' PLACEMENT OF MCR ',mcr_num,'(',HEX(mcr_num,3),') DOES NOT MATCH CONSTRAINT IN CONSTRAINT FILE ');
...
BEGIN
	...
	init;
	writeln;
	writeln('CVAX Pass 2 Allocator  V1.06  26-Sep-86');
